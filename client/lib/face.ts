...
function getModelsBase(): string {
  const fromEnv = (import.meta as any).env?.VITE_FACE_MODELS_URL as
    | string
    | undefined;
  return (
...
export async function checkLivenessFlexible(
  video: HTMLVideoElement,
  opts?: { tries?: number; intervalMs?: number; strict?: boolean },
) {
  ...
  for (let i = 0; i < tries; i++) {
    ...
    const ratio = (ear(leftEye) + ear(rightEye)) / 2;
    if (lastEyeRatio != null && Math.abs(ratio - lastEyeRatio) > 0.06)
      evidence++;
    lastEyeRatio = ratio;
    const nose = pts[30];
    if (lastNose && Math.hypot(nose.x - lastNose.x, nose.y - lastNose.y) > 4)
      evidence++;
