      ...
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={selectedBranchId ?? undefined} onValueChange={async (v) => {
            ...
            <SelectTrigger className="w-48"><SelectValue placeholder={tr("اختر الفرع", "Select branch")} /></SelectTrigger>
            <SelectContent>
              {Object.values(branches).map((b) => (<SelectItem key={b.id} value={b.id}>{b.name}</SelectItem>))}
            </SelectContent>
          </Select>
          <input className="ms-4 w-48 rounded-md border bg-background px-3 py-2 text-sm" placeholder={tr("ابحث بالاسم", "Search by name")} value={qDraft} onChange={(e)=>setQDraft(e.target.value)} />
          <button className="inline-flex items-center justify-center rounded-md bg-primary px-3 py-2 text-sm text-primary-foreground hover:bg-primary/90" onClick={()=>setQuery(qDraft)} type="button">{tr("بحث", "Search")}</button>
        </div>
      </div>

      <div className="rounded-xl border bg-card overflow-hidden">
        <table className="w-full text-right">
          <thead className="bg-secondary/50"><tr className="text-sm"><th className="p-3">{tr("الاسم", "Name")}</th><th className="p-3">{tr("تاريخ الوصول", "Arrival Date")}</th><th className="p-3">{tr("تاريخ الخروج", "Exit Date")}</th><th className="p-3">{tr("عدد عمليات التحقق", "Verifications")}</th><th className="p-3">{tr("الملف", "Profile")}</th><th className="p-3">{tr("آخر مبلغ", "Last Amount")}</th><th className="p-3">{tr("عرض", "View")}</th></tr></thead>
            ...
            {list.map((w) => {
              ...
              return (
                <tr key={w.id} className="hover:bg-secondary/40">
                    ...
                    <div className="flex flex-col">
                      ...
                      {(() => { const locked = !!w.exitDate && w.status !== "active"; if (!locked) return null; const pending = w.status === "unlock_requested"; return (
                        <div className="mt-1 flex items-center gap-2 text-xs">
                          <span className="inline-flex items-center rounded-full bg-rose-600/10 px-2 py-0.5 font-semibold text-rose-700">{tr("مقفولة", "Locked")}</span>
                          {pending ? <span className="text-muted-foreground">{tr("قيد انتظار الإدارة", "Pending admin")}</span> : null}
                        </div>
                      ); })()}
                    </div>
                  </td>
                  <td className="p-3 text-sm text-muted-foreground">{new Date(w.arrivalDate).toLocaleDateString(locale === "ar" ? "ar-EG" : "en-US")}</td>
                  <td className="p-3 text-sm text-muted-foreground">{w.exitDate ? new Date(w.exitDate).toLocaleDateString(locale === "ar" ? "ar-EG" : "en-US") : "—"}</td>
                  ...
                  <td className="p-3 text-sm"><Link to={`/workers/${w.id}`} className="text-primary hover:underline">{tr("تفاصيل", "Details")}</Link></td>
                </tr>
              );
            })}
            {list.length === 0 && (<tr><td colSpan={6} className="p-6 text-center text-muted-foreground">{tr("لا توجد عاملات في هذا الفرع.", "No workers in this branch.")}</td></tr>)}
          </tbody>
          <tfoot><tr className="bg-muted/40 font-semibold"><td className="p-3" colSpan={5}>{tr("إجمالي آخر المبالغ", "Total of last amounts")}</td><td className="p-3">₱ {totalLastPayments}</td><td /></tr></tfoot>
        </table>
      </div>
    </main>
  );
