...
export default function DownloadReport() {
  ...
  const assignedAreaFilters = useMemo(
    () =>
      selectedAreas
        .map((area) => area.trim())
  ...
  const assignedAreaFiltersLower = useMemo(
    () => assignedAreaFilters.map((area) => area.toLowerCase()),
    [assignedAreaFilters],
  );
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [branchId, fromTs, toTs, assignedAreaFiltersLower]);
